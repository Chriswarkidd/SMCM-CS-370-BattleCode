...
def move_and_shrink(unit):
	...
	for tup in moves:
		if gc.is_move_ready(unit.id) and gc.can_move(unit.id, bc.Direction(tup[1])):
			gc.move_robot(unit.id, bc.Direction(tup[1]))
			print('Moved successfully! move and shrink')
			continue
			
def random_move(unit):
	#map_of_earth = gc.starting_map(bc.Planet.Earth)
	#print(map_of_earth)
	dir = random.choice(directions)
	#a = unit.location.to_json()
	#a = MapLocation.from_json(a)
	#print(a.x, 'this is a x location', a.y, 'this is a y location')
	map_of_Earth = gc.starting_map(bc.Planet.Earth)
	height = map_of_Earth.height
	width = map_of_Earth.width
	tempCheck = []
	tempList = []
	for i in range(height):
		...
		for j in range(width):
			try:
				temp_loc = bc.MapLocation(bc.Planet.Earth, j, i)
				if map_of_Earth.is_passable_terrain_at(temp_loc):
					#have it append to list of 
					tempRow = tempRow + [0]
				else:
					tempRow = tempRow + [1]
					h = h + 1
				...
				# use this to show where the error was
				traceback.print_exc() 
		tempList = tempList + tempRow
		tempCheck = tempCheck + [h]
	for i in tempCheck:
		print(i)
	#for i in tempList:
	#	print(i)
	if gc.is_move_ready(unit.id) and gc.can_move(unit.id, dir):
		print(unit.location, 'this is the units location')
		gc.move_robot(unit.id, dir)
		print('Moved successfully! Random')
